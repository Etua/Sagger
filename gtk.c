...
void open_second_window(GtkWidget *first_window, char *second_window_label) {
  ...
  second_window =
      gtk_dialog_new_with_buttons("End", GTK_WINDOW(first_window), flags,
  ...
  g_signal_connect_swapped(second_window, "response",
                           G_CALLBACK(gtk_widget_destroy), second_window);
...
void open_info_window(GtkWidget *somewidget) {
  ...
  const gchar *authors[] = {"Dominik 'Etua' Danelski<dominik@danelski.pl>",
                            NULL};
  ...
  gtk_about_dialog_set_copyright(about,
                                 "Copyright © 2020 Dominik 'Etua' Danelski");
...
static void run_all(GtkWidget *somewidget, setup *passed_settings) {
  ...
  if (gtk_file_chooser_get_filename(
          GTK_FILE_CHOOSER(passed_settings->first_dir_wid)) == NULL ||
      gtk_file_chooser_get_filename(
          GTK_FILE_CHOOSER(passed_settings->second_dir_wid)) == NULL) {
    create_label(-3, error_label);
    open_second_window(passed_settings->first_window, error_label);
  } else {
    strcpy(final.f_dir_1, gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(
                              passed_settings->first_dir_wid)));
    strcpy(final.f_dir_2, gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(
                              passed_settings->second_dir_wid)));
    final.is_artist[0] = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(passed_settings->artist_1_wid));
    final.is_artist[1] = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(passed_settings->artist_2_wid));
    final.is_album[0] = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(passed_settings->album_1_wid));
    final.is_album[1] = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(passed_settings->album_2_wid));
...
void activate(GtkApplication *app) {
  ...
  directory1 = gtk_file_chooser_button_new(
      "Source directory", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  set_expand(directory1);
  gtk_container_add(GTK_CONTAINER(settings), directory1);
  directory2 = gtk_file_chooser_button_new(
      "Target directory", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  ...
  album_1 = gtk_radio_button_new_with_label_from_widget(
      GTK_RADIO_BUTTON(artist_1), "Album");
  gtk_container_add(GTK_CONTAINER(settings), album_1);
  both = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(artist_1),
                                                     "Artist and album");
  ...
  gtk_container_add(GTK_CONTAINER(settings), button);
  g_signal_connect(button, "clicked", G_CALLBACK(run_all),
                   (gpointer)&setup_run);
