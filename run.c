...
//TODO Include checks whether API key was changed and/or preferably more thoughtful way of storing it
...
void new_filename(run_settings new_filename_settings, recording_metadata *new_filename_metadata, char *second_dir,
                  new_names *set, char *special_name, _Bool retain_filename, char *old_name) {
    char inter[] = " - ";
...
//Makes new directories and moves files, returns FALSE for other errors than existing directory
_Bool change_name(char *source, new_names *to_change) {
    if(mkdir(to_change->dirname, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == -1 && errno != 17)
        return FALSE;
...
//Main function invoking other functions and coordinating data flow. It is not GUI-specific
void master_function(run_settings run_final, _Bool graphical) {
    ...
    recording_metadata *master_recording_metadata = malloc(sizeof(recording_metadata));
                ...
                if (access(file_p->fts_path, existence_test) == 0) {
                    ...
                    if (master_recording_metadata->song_name[0] == '^')
                        new_filename(run_final, master_recording_metadata, run_final.f_dir_2, master_names,
                                     "[unknown]", TRUE, file_p->fts_name);
                    else
                        new_filename(run_final, master_recording_metadata, run_final.f_dir_2, master_names,
                                     "none", FALSE, file_p->fts_name);
